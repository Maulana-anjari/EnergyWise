...
def insert_dummy_data():
    ...
    for device_id in range(1, 4):
        ...
        while current_time < datetime.now():
            energy_comsumption = round(random.uniform(1, 20), 2)
            energy_usage_data.append((device_id, current_time, energy_comsumption))
            current_time += timedelta(minutes=10)

    cursor.executemany("""
        INSERT INTO public.energy_usage_data (device_id, "timestamp", energy_comsumption)
        VALUES (%s, %s, %s)
    """, energy_usage_data)
